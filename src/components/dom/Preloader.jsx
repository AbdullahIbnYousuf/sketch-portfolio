...
const Preloader = ({ onComplete, ready }) => {
  ...
  // ----------------------------------------
  // SMOOTH FAST LOADING LOGIC
  // ----------------------------------------
  useEffect(() => {
    let newTarget = 0;
    if (active) {
      newTarget = (realProgress / 100) * 100;
    } else {
      newTarget = 100;
    }
  ...
  useEffect(() => {
    ...
    gsap.to(trackerRef.current, {
      ...
      onUpdate: () => {
        ...
        else if (safeProgress < 50) statusMsg = "Laying React & TypeScript Foundations...";
        else if (safeProgress < 70) statusMsg = "Connecting Python & AI Workflows...";
        else if (safeProgress < 85) statusMsg = "Structuring APIs & PostgreSQL Systems...";
  ...
  const startExit = () => {
    ...
    // 1. Instant tear apart (0.5s duration)
